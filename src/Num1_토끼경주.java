...
/**
 [입력] 테스트케이스는 파일로 주어진다.
 첫 번째 줄에 테스트케이스의 개수가 주어지고,
 각 테스트케이스의 첫번째 줄에는 경주장 10개의 지점의 높이 H가 주어진다.
 ( 0 <= Hi <= 100 )
 각 테스트케이스의 두 번째 줄부터 여섯번 째 줄 까지 각 줄마다 5마리의 토끼의
 오르막 한계치 A, 내리막 한계치 B가 공백을 두고 입력된다. ( 0<= B <= A <= 100 )
 [출력]
 테스트케이스 별로 #T (T는 테스트케이스의 번호)를 출력한 후 한 칸 띄워 경주장을
 완수할 수 있는 토끼의 수를 출력한다.
